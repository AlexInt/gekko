...
本目录是 Gekko Reborn 的产品/使用/运维/架构文档集合，面向当前代码仓库（`backend/` + `frontend/` + `docker-compose.yml`）。

如果你来自旧版 Gekko（`gekko-die`），可以把这里理解为对其 `docs/` 的现代化对应：目录结构保持相近，但内容以 **Gekko Reborn 的实际实现** 为准。

## 快速入口

- 介绍
  - [项目介绍](./introduction/about_gekko_reborn.md)
